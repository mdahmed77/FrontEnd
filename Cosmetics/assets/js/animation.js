$(document).ready(function () {
    ...
    var $animation_elements6 = $('.blurin');
    var $window6 = $(window);

    function check_if_in_view6() {
        var window_height6 = $window6.height();
        var window_top_position6 = $window6.scrollTop();
        var window_bottom_position6 = (window_top_position6 + window_height6);

        $.each($animation_elements6, function () {
            var $element6 = $(this);
            var element_height6 = $element6.outerHeight();
            var element_top_position6 = $element6.offset().top;
            var element_bottom_position6 = (element_top_position6 + element_height6);

            //check to see if this current container is within viewport
            if ((element_bottom_position6 >= window_top_position6) &&
                (element_top_position6 <= window_bottom_position6)) {
                $element6.addClass('focus');
            } else {
                $element6.removeClass(null);
            }
        });
    }

    $window6.on('scroll resize', check_if_in_view6);
    $window6.trigger('scroll');
});